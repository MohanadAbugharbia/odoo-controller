...
# odoo.abugharbia/odoo-operator-bundle:$VERSION and odoo.abugharbia/odoo-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= odoo.abugharbia/odoo-operator
...
IMG ?= ghcr.io/mohanadabugharbia/odoo-operator:$(VERSION)
...
build: manifests generate fmt vet ## Build manager binary.
	go build -o bin/odoo-operator cmd/main.go
...
docker-build: build ## Build docker image with the manager.
	$(CONTAINER_TOOL) build -t ${IMG} --build-arg TARGETPLATFORM=bin .
...
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	...
	$(KUSTOMIZE) build config/default > out/install.yaml
